jQuery(document).ready(function($) {
    ...
    jQuery('.news .add-slide').on('click', function(event) {
        ...
        add_slide_frame.on('insert', function() {
            jQuery(".news .spinner").show();
            jQuery(".news input[type=submit]").attr('disabled', 'disabled');
            ...
            var data = {
                ...
                slider_id: news_slider_id,
                selection: slide_ids,
                _wpnonce: news.addslide_nonce
            };
            jQuery.post(news.ajaxurl, data, function(response) {
                jQuery(".news .left table").append(response);
                jQuery(".news .left table").trigger('resizeSlides');
            });
        });
        add_slide_frame.open();
    ...
    jQuery('.news .change-image').on('click', function(event) {
        ...
        change_slide_frame.on('select', function() {
            ...
            var data = {
                ...
                _wpnonce: news.changeslide_nonce,
    ...
    var switchType = function(slider) {
        jQuery('.news .option:not(.' + slider + ')').attr('disabled', 'disabled').parents('tr').hide();
        jQuery('.news .option.' + slider).removeAttr('disabled').parents('tr').show();
        jQuery('.news input.radio:not(.' + slider + ')').attr('disabled', 'disabled');
        jQuery('.news input.radio.' + slider).removeAttr('disabled');
        jQuery('.news .showNextWhenChecked:visible').parent().parent().next('tr').hide();
        jQuery('.news .showNextWhenChecked:visible:checked').parent().parent().next('tr').show();
    ...
    switchType(jQuery(".news .select-slider:checked").attr("rel"));
    ...
    toggleNextRow(jQuery(".news .showNextWhenChecked"));
    jQuery(".news .showNextWhenChecked").on("change", function() {
        toggleNextRow(jQuery(this));
    });
    // mark the slide for resizing when the crop position has changed
    jQuery(".news").on('change', '.left tr.slide .crop_position', function() {
        jQuery(this).closest('tr').data('crop_changed', true);
    });
    // handle slide libary switching
    jQuery(".news .select-slider").on("click", function() {
        switchType(jQuery(this).attr("rel"));
    });
    // return a helper with preserved width of cells
    var news_sortable_helper = function(e, ui) {
    ...
    jQuery(".news .left table tbody").sortable({
        helper: news_sortable_helper,
        handle: "td.col-1",
        stop: function() {
            jQuery(".news .left table").trigger("updateSlideOrder");
            jQuery(".news form #ms-save").click();
        }
    });
    // bind an event to the slides table to update the menu order of each slide
    jQuery(".news .left table").live("updateSlideOrder", function(event) {
        jQuery("tr", this).each(function() {
            jQuery("input.menu_order", jQuery(this)).val(jQuery(this).index());
        });
    });
    // bind an event to the slides table to update the menu order of each slide
    jQuery(".news .left table").live("resizeSlides", function(event) {
        ...
        jQuery("tr.slide input[name='resize_slide_id']", this).each(function() {
            ...
            if (thumb_width != slideshow_width || thumb_height != slideshow_height || crop_changed === true) {
                ...
                var data = {
                    ...
                    slider_id: window.parent.news_slider_id,
                    slide_id: $this.attr("data-slide_id"),
                    _wpnonce: news.resize_nonce
                };
                jQuery.ajax({
                    type: "POST",
                    data: data,
    ...
    jQuery(document).ajaxStop(function() {
        jQuery(".news .spinner").hide();
        jQuery(".news input[type=submit]").removeAttr("disabled");
    });
    jQuery(".useWithCaution").on("change", function() {
        if (!this.checked) {
            return alert(news.useWithCaution);
        }
    });
    // helptext tooltips
    ...
    jQuery(".news .shortcode input, .news .shortcode textarea").on('click', function() {
        this.select();
    });
    ...
    jQuery(".news .ms-toggle .hndle, .news .ms-toggle .handlediv").on('click', function() {
        jQuery(this).parent().toggleClass('closed');
    });
    jQuery(".news").on('click', 'ul.tabs li', function() {
        var tab = jQuery(this);
    ...
    jQuery(".news").on('click', '.delete-slider', function() {
        return confirm(news.confirm);
    });
    // delete a slide using ajax (avoid losing changes)
    jQuery(".news").on('click', '.delete-slide', function(e) {
        e.preventDefault();
        var link = jQuery(this);
        if (confirm(news.confirm)) {
    ...
    // AJAX save & preview
    jQuery(".news form").find("input[type=submit]").on("click", function(e) {
        e.preventDefault();
        jQuery(".news .spinner").show();
        jQuery(".news input[type=submit]").attr("disabled", "disabled");
        // update slide order
        ...
        jQuery.ajax({
            ...
            success: function(data) {
                ...
                jQuery.when(jQuery(".news .left table").trigger("resizeSlides")).done(function() {
                    ...
                    if (button.id === "ms-preview") {
                        jQuery.colorbox({
                            iframe: true,
                            href: news.iframeurl + "&slider_id=" + jQuery(button).data("slider_id"),
                            transition: "elastic",
